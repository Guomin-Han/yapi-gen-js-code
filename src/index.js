...
async function getInterfaceList(config){
  ...
  try{
    result = await axios.get (
      `${config.server}/api/interface/${config.categoryId ? 'list_cat' : 'list'}?limit=10000&token=${config.token}&catid=${config.categoryId}`
    );
...
async function gen (options = {}) {
  ...
  for(let i=0; i< list.length; i++){
    ...
    interfaceData.native_path = interfaceData.path;
